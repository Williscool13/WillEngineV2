...
void EnvironmentDescriptorLayouts::createLayouts()
{
    //  Equirectangular Image Descriptor Set Layout
    {
        ...
        equiImageLayout = layoutBuilder.build(context.device, VK_SHADER_STAGE_FRAGMENT_BIT | VK_SHADER_STAGE_COMPUTE_BIT
    ...
    //  (Storage) Cubemap Descriptor Set Layout
    {
        ...
        cubemapStorageLayout = layoutBuilder.build(context.device, VK_SHADER_STAGE_COMPUTE_BIT
                                                   , nullptr, VK_DESCRIPTOR_SET_LAYOUT_CREATE_DESCRIPTOR_BUFFER_BIT_EXT);
    }
    //  (Sampler) Cubemap Descriptor Set Layout
    {
        DescriptorLayoutBuilder layoutBuilder;
        layoutBuilder.addBinding(0, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
        cubemapSamplerLayout = layoutBuilder.build(context.device, VK_SHADER_STAGE_COMPUTE_BIT | VK_SHADER_STAGE_FRAGMENT_BIT
                                                   , nullptr, VK_DESCRIPTOR_SET_LAYOUT_CREATE_DESCRIPTOR_BUFFER_BIT_EXT);
    }
    // LUT generation
    {
        DescriptorLayoutBuilder layoutBuilder;
        layoutBuilder.addBinding(0, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE);
        lutLayout = layoutBuilder.build(context.device, VK_SHADER_STAGE_COMPUTE_BIT
                                        , nullptr, VK_DESCRIPTOR_SET_LAYOUT_CREATE_DESCRIPTOR_BUFFER_BIT_EXT);
    }
    ...
    // Full Cubemap Descriptor - contains PBR IBL data
    {
        ...
        environmentMapLayout = layoutBuilder.build(context.device, VK_SHADER_STAGE_FRAGMENT_BIT | VK_SHADER_STAGE_COMPUTE_BIT
                                                   , nullptr, VK_DESCRIPTOR_SET_LAYOUT_CREATE_DESCRIPTOR_BUFFER_BIT_EXT);
    }
}
