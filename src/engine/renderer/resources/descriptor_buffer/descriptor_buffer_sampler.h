...
public:
    ...
    static DescriptorBufferSampler create(const VulkanContext& ctx, VkDescriptorSetLayout descriptorSetLayout, int32_t maxObjectCount = 10);
    ...
    DescriptorBufferSampler(DescriptorBufferSampler&& other) noexcept = default;
    DescriptorBufferSampler& operator=(DescriptorBufferSampler&& other) noexcept = default;
     ...
     */
    int32_t setupData(const VulkanContext& ctx, const std::vector<DescriptorImageData>& imageBuffers, int32_t index = -1);
